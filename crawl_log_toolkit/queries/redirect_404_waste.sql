...
-- Note:    304 Not Modified is NOT counted -- it is healthy conditional-GET
--          caching, not waste. See `redirect_404_urls` for the offending paths.
--
-- Dialect notes:
--   BigQuery / Snowflake : COUNTIF / COUNT(CASE WHEN ...).
--   ClickHouse           : countIf(status BETWEEN 300 AND 399 AND status != 304).
SELECT
    COUNT(*)                                                            AS total_crawls,
    COUNT(*) FILTER (WHERE status BETWEEN 300 AND 399 AND status <> 304) AS redirects,
    COUNT(*) FILTER (WHERE status = 404)                                AS not_found,
    COUNT(*) FILTER (WHERE status BETWEEN 500 AND 599)                  AS server_errors,
    ROUND(100.0 * COUNT(*) FILTER (
        WHERE (status BETWEEN 300 AND 399 AND status <> 304) OR status = 404
